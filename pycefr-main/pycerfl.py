...
def request_url():
    ...
    # Check language
    check_language(option, protocol, type_git, user, repo)
...
def run_user():
    """ Run user. """
    # Create the url of the api
    user_url = f"https://api.github.com/users/{option}"
    print(user_url)
    print("Analyzing user...\n")
    try:
        # Extract headers
        headers = requests.get(user_url, timeout=5)
        # Decode JSON response into a Python dict:
    ...
    names = requests.get(repo_url, timeout=5)
    # Decode JSON response into a Python dict:
    content = names.json()
    # Show repository names
    for repository in content:
        print(f'\nRepository: {repository["name"]}')
        url = f"https://github.com/{option}/{repository['name']}"
        check_language(url, 'https', 'github.com', option, repository['name'])
...
def get_directory(url):
    ...
    print(f"The directory is: {name_directory}")
...
def get_path(name_directory):
    ...
    fiche = abs_file_path.split('/')[-1]
    if fiche.endswith('.py'):
        abs_file_path = abs_file_path.replace("/" + fiche, "")
    print("This script absolute path is ", abs_file_path)
