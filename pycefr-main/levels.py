...
def type_Call(self):
    ...
    if 'ast.Attribute' in str(self.node.func):
        ...
        elif (self.node.func.id) == 'str':
            value = 'str'
            level_Str(self, value)
        elif (self.node.func.id) == 'int':
            value = 'int'
            level_Int(self, value)
        elif (self.node.func.id) == 'float':
            value = 'float'
            level_Float(self, value)
...
def level_Float(self, value):
    """ Float level. """
    self.level = dictLevel['Float'][0]['simple']
    self.clase = 'Float'

def level_Str(self, value):
    """ Str level. """
    self.level = dictLevel['Str'][0]['simple']
    self.clase = 'Str'

def level_Int(self, value):
    """ Int level. """
    self.level = dictLevel['Int'][0]['simple']
    self.clase = 'Int'
